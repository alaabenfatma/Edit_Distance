...
def alignment(s1, s2, mat, color=True):
    color_red = '\033[91m'
    color_blue = '\033[94m'
    ...
    if color == False:
        color_red = color_blue = color_purple = color_end = ''
...
def recursive(string1, string2):
    matrix = prepare_mat(len(string1), len(string2))
    ed = ed_with_alignement(string1, string2, matrix)
    align = alignment(string1, string2, matrix, False)
    return ed, align
